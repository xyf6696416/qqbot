...
│   ├── plugin_base.py   # PluginBase 基类 + 装饰器 + 事件对象
│   ├── plugin_manager.py# PluginManager 生命周期管理
...
开发进度和 TODO 见 **[docs/progress.md](docs/progress.md)**：

- **阶段三**: `@cron` 定时任务装饰器 / LLM 请求拦截 / 现有功能提取为内置插件
- **阶段四**: 插件市场 / 配置 UI schema / 性能监控 / 进程隔离

### 2026-06-15 — 插件系统 + 热加载 + Web 管理
- **插件系统阶段一**：`mod/` 核心框架（PluginBase + EventBus + PluginManager + PluginContext + 4 个装饰器）
- **插件系统阶段二**：mtime 热加载 + Web 面板「插件管理」选项卡（列表/重载/启用禁用）
- 移除 SF 意图兜底，低置信度直接走 chat
- 新增 `image_dedup.py` — 图片去重模块
- 新增 `start_bridge.bat` / `start_config_web.bat` — 启动脚本
- 完善 `.gitignore`，排除缓存/日志/备份文件
